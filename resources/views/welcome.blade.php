...
:root{
  ...
  --r2:20px;
  --r3:28px;
  --sh:0 2px 24px rgba(8,8,15,.06);
  --sh2:0 8px 56px rgba(67,56,202,.14);
  --sh3:0 20px 80px rgba(8,8,15,.18);
...
.nav-links{display:flex;align-items:center;gap:1px;margin-left:auto;margin-right:16px}
.nav-links a{font-size:13.5px;font-weight:600;color:var(--muted);padding:8px 13px;border-radius:9px;transition:all var(--trans);white-space:nowrap}
...
.nav-cta{
  ...
  color:#fff;padding:11px 22px;border-radius:10px;font-weight:700;font-size:13.5px;
...
.mobile-menu .mob-cta{
  ...
  text-align:center;border-radius:14px;border:none;padding:18px;margin-top:12px;font-size:16px;
}

/* ═══ HERO ═══ */
.hero{position:relative;min-height:100svh;display:flex;align-items:center;overflow:hidden}
.hero-bg{position:absolute;inset:0;width:100%;height:100%;object-fit:cover;z-index:0;transform:scale(1.05);transition:transform 10s ease-out}
.hero-bg.loaded{transform:scale(1)}
.hero-ov1{position:absolute;inset:0;z-index:1;background:linear-gradient(135deg,rgba(8,8,15,.92) 0%,rgba(29,78,216,.7) 45%,rgba(109,40,217,.5) 100%)}
.hero-ov2{position:absolute;inset:0;z-index:2;background:linear-gradient(to bottom,transparent 60%,rgba(8,8,15,.6) 100%)}
/* Animated grain */
.hero-grain{
  position:absolute;inset:0;z-index:3;opacity:.025;
  background-image:url("data:image/svg+xml,%3Csvg viewBox='0 0 512 512' xmlns='http://www.w3.org/2000/svg'%3E%3Cfilter id='n'%3E%3CfeTurbulence type='fractalNoise' baseFrequency='0.75' numOctaves='4' stitchTiles='stitch'/%3E%3C/filter%3E%3Crect width='100%25' height='100%25' filter='url(%23n)'/%3E%3C/svg%3E");
  background-size:200px;animation:grainMove 8s steps(10) infinite;
}
...
.hero h1 .line-accent{
  background:linear-gradient(90deg,var(--violet3),#fff);
...
.sec-h{
  font-family:'Playfair Display',serif;
  font-size:clamp(32px,4.5vw,50px);color:var(--ink);
...
.about-img-main{
  width:100%;height:480px;object-fit:cover;border-radius:var(--r3);
  box-shadow:var(--sh3);
}
.about-img-sm{
  position:absolute;bottom:-28px;right:-28px;
  width:190px;height:190px;object-fit:cover;
  border-radius:var(--r2);border:5px solid var(--white);
  box-shadow:var(--sh3);
}
.about-badge{
...
.chip-sm{
  background:var(--warm);color:var(--violet);
...
.jenjang-card::before{
  content:'';position:absolute;inset:0;
  background:linear-gradient(135deg,rgba(29,78,216,.04),rgba(109,40,217,.06));
...
.jenjang-detail{
  ...
  cursor:pointer;transition:gap var(--trans);
}
.jenjang-card:hover .jenjang-detail{gap:9px}
/* Detail panel */
.jenjang-expand{
  background:var(--warm);border-radius:12px;padding:16px;
...
.prog-overlay{
  ...
  background:linear-gradient(to top,rgba(8,8,15,.7) 0%,transparent 50%);
  opacity:0;transition:opacity var(--trans);
}
...
.prog-tag{
  ...
  background:var(--warm);color:var(--violet);
  font-size:11px;font-weight:800;padding:4px 13px;border-radius:99px;
  letter-spacing:.05em;margin-bottom:12px;border:1px solid rgba(109,40,217,.15);
}
...
.ku-card{
  ...
  border-radius:var(--r2);padding:32px 20px 28px;
...
.ku-card::before{
  content:'';position:absolute;inset:0;
  background:linear-gradient(135deg,rgba(109,40,217,.15),rgba(29,78,216,.1));
  opacity:0;transition:opacity var(--trans);
}
...
.ku-num{
  ...
  display:flex;align-items:center;justify-content:center;
  border:2px solid rgba(8,8,15,.8);
}
...
.testi-quote-mark{
  font-family:'Playfair Display',serif;font-size:72px;line-height:.7;
  color:var(--violet2);opacity:.15;position:absolute;top:20px;right:22px;
}
...
.galeri-item:first-child img{height:100%;min-height:454px}
.galeri-ov{
  position:absolute;inset:0;
...
.galeri-item:hover .galeri-ov{opacity:1}
.galeri-ov span{color:#fff;font-size:13.5px;font-weight:700}

/* ═══ TUTOR ═══ */
.tutor-sec{background:var(--cream)}
.tutor-grid{display:grid;grid-template-columns:repeat(5,1fr);gap:22px}
.tutor-card{
  text-align:center;transition:all var(--trans);
  background:var(--white);border-radius:var(--r2);
  padding:28px 18px 24px;border:1.5px solid var(--border);
  position:relative;overflow:hidden;
}
.tutor-card::before{
...
.tutor-badge{
  ...
  display:flex;align-items:center;justify-content:center;
  border:2px solid var(--white);font-weight:900;
}
.tutor-name{font-weight:800;font-size:14.5px;color:var(--ink);margin-bottom:5px}
.tutor-subj{
  font-size:12px;color:var(--violet);font-weight:700;margin-bottom:8px;
  background:var(--warm);padding:3px 12px;border-radius:99px;display:inline-block;
}
.tutor-rating{font-size:13.5px;color:var(--gold2);font-weight:800;margin-bottom:4px}
...
.faq-q{
  ...
  cursor:pointer;font-weight:700;font-size:14.5px;color:var(--ink);gap:14px;
}
.faq-ico{
  width:28px;height:28px;border-radius:50%;flex-shrink:0;
  background:var(--white);color:var(--violet);border:1.5px solid var(--border);
...
/* Form */
.contact-box{
  background:linear-gradient(140deg,#0F172A 0%,#1D2B6B 50%,#2D1B69 100%);
  border-radius:var(--r3);padding:clamp(28px,4vw,42px);
  position:relative;overflow:hidden;
}
...
.contact-box-title{
  font-family:'Playfair Display',serif;
  font-size:28px;color:#fff;margin-bottom:8px;position:relative;z-index:1;
}
...
.f-group input,.f-group select,.f-group textarea{
  ...
  font-family:'Plus Jakarta Sans',sans-serif;font-size:14px;color:#fff;outline:none;
...
.btn-submit{
  ...
  color:var(--ink);padding:16px;border-radius:12px;border:none;
...
.cabang-btn{
  ...
  padding:7px 18px;border-radius:9px;font-size:12.5px;font-weight:700;
...
footer{
  ...
  padding:clamp(56px,8vw,88px) clamp(20px,5vw,64px) 36px;
}
...
#wa-float{
  ...
  background:#1DA851;color:#fff;border-radius:56px;
  padding:15px 24px;display:flex;align-items:center;gap:10px;
  font-weight:800;font-size:14px;
  box-shadow:0 8px 40px rgba(29,168,81,.5);
...
/* ═══ NOTIF ═══ */
#notif{
  position:fixed;bottom:100px;left:24px;z-index:9001;
  background:var(--white);border-radius:18px;
  box-shadow:0 8px 56px rgba(0,0,0,.16);
  padding:16px 20px;display:flex;align-items:center;gap:14px;
  max-width:300px;border-left:4px solid var(--violet);
...
#modal{
  ...
  background:rgba(8,8,15,.6);backdrop-filter:blur(6px);
  display:flex;align-items:center;justify-content:center;
...
.modal-box{
  ...
  transform:scale(.86);transition:transform .35s cubic-bezier(.34,1.56,.64,1);
}
...
.modal-btn{
  background:linear-gradient(135deg,var(--indigo),var(--violet));
  color:#fff;padding:14px 40px;border-radius:12px;border:none;
  font-family:'Plus Jakarta Sans',sans-serif;font-size:15px;font-weight:800;
  cursor:pointer;transition:all var(--trans);box-shadow:0 6px 20px rgba(109,40,217,.35);
}
...
@media(max-width:1100px){
  ...
  .galeri-item:first-child img{min-height:300px}
}
@media(max-width:768px){
  ...
  .galeri-item:first-child img{min-height:220px}
...
<!-- ══════════════════ HERO ══════════════════ -->
<section class="hero">
  <img src="/banner.jpeg" class="hero-bg" id="heroBg" alt="Siswa belajar SCI" loading="eager">
    ...
    <div class="about-grid reveal">
      <div class="about-imgs">
        ...
        <div class="about-badge">
          <strong>#1</strong>
          <span>Bimbel<br>Terpercaya</span>
        </div>
      </div>
  ...
  <div class="sec-inner">
    ...
    <div class="jenjang-grid reveal">
      ...
      <div class="jenjang-card" onclick="toggleJenjang(this)">
        <div class="jenjang-img-wrap">
          <img src="https://images.unsplash.com/photo-1503454537195-1dcabb73ffb9?w=200&h=200&q=80&auto=format&fit=crop" class="jenjang-img" alt="TK" loading="lazy">
          <div class="jenjang-num">1</div>
        </div>
        <h3>TK</h3>
        <p>Taman Kanak-Kanak</p>
        <div class="jenjang-detail">Lihat Detail <span>→</span></div>
        <div class="jenjang-expand">
          <ul>
            <li>Calistung (Baca Tulis Hitung)</li>
            <li>Pengenalan Angka &amp; Huruf</li>
            <li>Seni &amp; Kreativitas Anak</li>
            <li>Persiapan Masuk SD</li>
          </ul>
        </div>
      </div>
      ...
      <div class="jenjang-card" onclick="toggleJenjang(this)">
        <div class="jenjang-img-wrap">
          <img src="https://images.unsplash.com/photo-1485546246426-74dc88dec4d9?w=200&h=200&q=80&auto=format&fit=crop" class="jenjang-img" alt="SD" loading="lazy">
          <div class="jenjang-num">2</div>
        </div>
        <h3>SD</h3>
        <p>Sekolah Dasar</p>
        <div class="jenjang-detail">Lihat Detail <span>→</span></div>
      ...
      <div class="jenjang-card" onclick="toggleJenjang(this)">
        <div class="jenjang-img-wrap">
          <img src="https://images.unsplash.com/photo-1427504494785-3a9ca7044f45?w=200&h=200&q=80&auto=format&fit=crop" class="jenjang-img" alt="SMP" loading="lazy">
          <div class="jenjang-num">3</div>
        </div>
        <h3>SMP</h3>
        ...
        <div class="jenjang-expand">
          <ul>
            ...
            <li>Biologi, Sejarah, Geografi</li>
            <li>Bahasa Inggris Intensif</li>
            <li>Persiapan PPDB SMA Favorit</li>
          </ul>
        </div>
      </div>
      ...
      <div class="jenjang-card" onclick="toggleJenjang(this)">
        <div class="jenjang-img-wrap">
          <img src="https://images.unsplash.com/photo-1523580494863-6f3031224c94?w=200&h=200&q=80&auto=format&fit=crop" class="jenjang-img" alt="SMA" loading="lazy">
          <div class="jenjang-num">4</div>
        </div>
        <h3>SMA / Umum</h3>
        <p>SMA &amp; Karyawan</p>
        <div class="jenjang-detail">Lihat Detail <span>→</span></div>
        <div class="jenjang-expand">
          <ul>
            ...
            <li>Kursus Profesional Dewasa</li>
            <li>Bahasa, Komputer, Akuntansi</li>
            <li>Persiapan CPNS &amp; Tes Kerja</li>
          </ul>
        </div>
      </div>
    ...
    <div class="program-grid reveal">
      ...
      <div class="prog-card" onclick="toggleProg(this)">
        <div class="prog-img-wrap">
          <img src="https://images.unsplash.com/photo-1518133910546-b6c2fb7d79e3?w=600&h=280&q=80&auto=format&fit=crop" class="prog-img" alt="Bimbel" loading="lazy">
          <div class="prog-overlay"></div>
        </div>
        <div class="prog-body">
          <span class="prog-tag">SEMUA JENJANG</span>
          <h3>Bimbel Mata Pelajaran</h3>
          <p>Bimbingan semua mata pelajaran sekolah dengan metode efektif dan menyenangkan.</p>
          <div class="prog-detail-btn">Lihat Detail <span>↓</span></div>
        </div>
        <div class="prog-detail-panel">
          <div class="prog-detail-inner">
            <h4>📋 Yang Kamu Dapatkan:</h4>
            <ul>
              <li>Semua mapel dari TK hingga SMA/Kuliah</li>
              <li>Sesi privat 1-on-1 atau kelompok kecil</li>
              <li>Laporan perkembangan tiap bulan</li>
              <li>Bisa home visit atau di kantor SCI</li>
      ...
      <div class="prog-card" onclick="toggleProg(this)">
        <div class="prog-img-wrap">
          <img src="https://images.unsplash.com/photo-1456513080510-7bf3a84b82f8?w=600&h=280&q=80&auto=format&fit=crop" class="prog-img" alt="Ujian" loading="lazy">
          <div class="prog-overlay"></div>
        </div>
        <div class="prog-body">
          <span class="prog-tag">SMP · SMA</span>
          <h3>Persiapan Ujian</h3>
          <p>Persiapan UTS, UAS &amp; Ujian Sekolah agar nilai meningkat pesat dan lulus terbaik.</p>
          <div class="prog-detail-btn">Lihat Detail <span>↓</span></div>
        </div>
        <div class="prog-detail-panel">
          <div class="prog-detail-inner">
            <h4>📋 Yang Kamu Dapatkan:</h4>
            <ul>
              <li>Materi fokus sesuai kisi-kisi ujian</li>
              <li>Try out berkala dengan pembahasan</li>
              <li>Strategi menjawab soal cepat &amp; tepat</li>
              <li>Mentoring intensif H-30 ujian</li>
              <li>Garansi nilai meningkat minimal 15%</li>
            </ul>
          </div>
        </div>
      </div>
      ...
      <div class="prog-card" onclick="toggleProg(this)">
        <div class="prog-img-wrap">
          <img src="https://images.unsplash.com/photo-1434030216411-0b793f4b4173?w=600&h=280&q=80&auto=format&fit=crop" class="prog-img" alt="Tes Masuk" loading="lazy">
          <div class="prog-overlay"></div>
        </div>
        <div class="prog-body">
          <span class="prog-tag">INTENSIF</span>
          <h3>Persiapan Tes &amp; SBMPTN</h3>
          <p>Persiapan masuk sekolah favorit, PTN, CPNS &amp; tes lainnya secara intensif.</p>
          <div class="prog-detail-btn">Lihat Detail <span>↓</span></div>
        </div>
        <div class="prog-detail-panel">
          <div class="prog-detail-inner">
            <h4>📋 Yang Kamu Dapatkan:</h4>
            <ul>
              <li>UTBK/SNBT, PPDB, CPNS, BUMN</li>
              <li>Modul soal eksklusif &amp; terbaru</li>
              <li>Simulasi tes online terjadwal</li>
              <li>Konsultasi pemilihan jurusan PTN</li>
              <li>Rekam jejak alumnus lulus PTN terbaik</li>
            </ul>
          </div>
        </div>
      </div>
      ...
      <div class="prog-card" onclick="toggleProg(this)">
        <div class="prog-img-wrap">
          <img src="https://images.unsplash.com/photo-1546410531-bb4caa6b424d?w=600&h=280&q=80&auto=format&fit=crop" class="prog-img" alt="Bahasa" loading="lazy">
          <div class="prog-overlay"></div>
        </div>
        <div class="prog-body">
          <span class="prog-tag">SEMUA LEVEL</span>
          <h3>Kursus Bahasa</h3>
          <p>Inggris, Jepang, Mandarin, Arab — tingkatkan kemampuan bahasa Anda bersama kami.</p>
          <div class="prog-detail-btn">Lihat Detail <span>↓</span></div>
        </div>
        <div class="prog-detail-panel">
          <div class="prog-detail-inner">
            <h4>📋 Yang Kamu Dapatkan:</h4>
            <ul>
              <li>Bahasa Inggris, Jepang, Mandarin, Arab</li>
              <li>Speaking, Writing, Grammar, Listening</li>
              <li>Sertifikat kelulusan resmi SCI</li>
              <li>Persiapan TOEFL, IELTS, JLPT, HSK</li>
              <li>Native &amp; bilingual tutor tersedia</li>
            </ul>
          </div>
        </div>
      </div>
      ...
      <div class="prog-card" onclick="toggleProg(this)">
        <div class="prog-img-wrap">
          <img src="https://images.unsplash.com/photo-1531297484001-80022131f5a1?w=600&h=280&q=80&auto=format&fit=crop" class="prog-img" alt="Komputer" loading="lazy">
          <div class="prog-overlay"></div>
        </div>
        <div class="prog-body">
          <span class="prog-tag" style="background:rgba(217,119,6,.1);color:#B45309;border-color:rgba(217,119,6,.2)">POPULER 🔥</span>
          <h3>Kursus Komputer</h3>
          <p>Microsoft Office, Desain Grafis, Programming — teknologi terkini untuk karir masa depan.</p>
          <div class="prog-detail-btn">Lihat Detail <span>↓</span></div>
        </div>
        <div class="prog-detail-panel">
          <div class="prog-detail-inner">
            ...
            <ul>
              ...
              <li>Desain Grafis: Photoshop, Canva, Figma</li>
              <li>Programming: Web, Python, Scratch</li>
              <li>Sertifikat resmi berskala nasional</li>
              <li>Langsung praktek &amp; project nyata</li>
            </ul>
          </div>
        </div>
      </div>
      ...
      <div class="prog-card" onclick="toggleProg(this)">
        <div class="prog-img-wrap">
          <img src="https://images.unsplash.com/photo-1554224155-6726b3ff858f?w=600&h=280&q=80&auto=format&fit=crop" class="prog-img" alt="Akuntansi" loading="lazy">
          <div class="prog-overlay"></div>
        </div>
        <div class="prog-body">
          <span class="prog-tag" style="background:rgba(5,150,105,.1);color:#047857;border-color:rgba(5,150,105,.2)">TERBARU ✨</span>
          <h3>Kursus Akuntansi</h3>
          <p>Akuntansi dasar hingga profesional, perpajakan &amp; keuangan untuk mahasiswa dan karyawan.</p>
          <div class="prog-detail-btn">Lihat Detail <span>↓</span></div>
        </div>
        <div class="prog-detail-panel">
          <div class="prog-detail-inner">
            <h4>📋 Yang Kamu Dapatkan:</h4>
            <ul>
              <li>Akuntansi Dasar hingga Profesional</li>
              <li>Perpajakan (PPh, PPN, e-Faktur)</li>
              <li>Software Akuntansi: MYOB, Accurate</li>
              <li>Laporan Keuangan &amp; Analisis Bisnis</li>
              <li>Sertifikat kompetensi akuntansi</li>
            </ul>
          </div>
        </div>
      </div>
...
<section class="sec keunggulan-sec" id="keunggulan">
  <div class="sec-inner">
    ...
    <div class="keunggulan-grid reveal">
      ...
      <div class="ku-card">
        <div class="ku-icon-wrap">
          <img src="https://images.unsplash.com/photo-1544717305-2782549b5136?w=150&h=150&q=80&auto=format&fit=crop&crop=face" class="ku-img" alt="Tutor Profesional" loading="lazy">
          <div class="ku-num">1</div>
        </div>
      ...
      <div class="ku-card">
        <div class="ku-icon-wrap">
          <img src="https://images.unsplash.com/photo-1558618666-fcd25c85cd64?w=150&h=150&q=80&auto=format&fit=crop" class="ku-img" alt="Home Visit" loading="lazy">
          <div class="ku-num">2</div>
        </div>
      ...
      <div class="ku-card">
        <div class="ku-icon-wrap">
          <img src="https://images.unsplash.com/photo-1516321318423-f06f85e504b3?w=150&h=150&q=80&auto=format&fit=crop" class="ku-img" alt="Metode Modern" loading="lazy">
          <div class="ku-num">3</div>
        </div>
        <h3>Metode Modern</h3>
      ...
      <div class="ku-card">
        <div class="ku-icon-wrap">
          <img src="https://images.unsplash.com/photo-1551288049-bebda4e38f71?w=150&h=150&q=80&auto=format&fit=crop" class="ku-img" alt="Hasil Terukur" loading="lazy">
          <div class="ku-num">4</div>
        </div>
        <h3>Hasil Terukur</h3>
      ...
      <div class="ku-card">
        <div class="ku-icon-wrap">
          <img src="https://images.unsplash.com/photo-1522071820081-009f0129c71c?w=150&h=150&q=80&auto=format&fit=crop" class="ku-img" alt="Support Penuh" loading="lazy">
          <div class="ku-num">5</div>
        </div>
        <h3>Support Penuh</h3>
    ...
    <div class="testi-grid reveal">
      <div class="testi-card">
        ...
        <div class="testi-author">
          <img src="https://images.unsplash.com/photo-1494790108377-be9c29b29330?w=100&h=100&q=80&auto=format&fit=crop&crop=face" class="testi-av" alt="Aisyah" loading="lazy">
          <div>
            <div class="testi-name">Aisyah Rahma</div>
            <div class="testi-role">Siswa SMA · Matematika</div>
            <div class="testi-verified">Siswa Terverifikasi</div>
          </div>
        </div>
      </div>
      <div class="testi-card">
        ...
        <div class="testi-author">
          <img src="https://images.unsplash.com/photo-1500648767791-00dcc994a43e?w=100&h=100&q=80&auto=format&fit=crop&crop=face" class="testi-av" alt="Ricky" loading="lazy">
          <div>
            <div class="testi-name">Ricky Pratama</div>
            <div class="testi-role">Mahasiswa · Persiapan SBMPTN</div>
            <div class="testi-verified">Siswa Terverifikasi</div>
          </div>
        </div>
      </div>
      <div class="testi-card">
        ...
        <div class="testi-author">
          <img src="https://images.unsplash.com/photo-1438761681033-6461ffad8d80?w=100&h=100&q=80&auto=format&fit=crop&crop=face" class="testi-av" alt="Dinda" loading="lazy">
          <div>
            <div class="testi-name">Dinda Lestari</div>
            <div class="testi-role">Mahasiswi · Akuntansi</div>
            <div class="testi-verified">Siswa Terverifikasi</div>
          </div>
        </div>
      </div>
  ...
  <div class="sec-inner">
    ...
    <div class="galeri-grid reveal">
      <div class="galeri-item">
        <img src="https://images.unsplash.com/photo-1580582932707-520aed937b7b?w=800&q=80&auto=format&fit=crop" alt="Kelas SCI" loading="lazy">
        <div class="galeri-ov"><span>Kelas Belajar Intensif</span></div>
      </div>
      <div class="galeri-item">
        <img src="https://images.unsplash.com/photo-1509062522246-3755977927d7?w=600&q=80&auto=format&fit=crop" alt="Tutor" loading="lazy">
        <div class="galeri-ov"><span>Sesi Les Privat</span></div>
      </div>
      <div class="galeri-item">
        <img src="https://images.unsplash.com/photo-1522202176988-66273c2fd55f?w=600&q=80&auto=format&fit=crop" alt="Group" loading="lazy">
        <div class="galeri-ov"><span>Belajar Kelompok</span></div>
      </div>
      <div class="galeri-item">
        <img src="https://images.unsplash.com/photo-1571260899304-425eee4c7efc?w=600&q=80&auto=format&fit=crop" alt="Komputer" loading="lazy">
        <div class="galeri-ov"><span>Kursus Komputer</span></div>
      </div>
      <div class="galeri-item">
        <img src="https://images.unsplash.com/photo-1524178232363-1fb2b075b655?w=600&q=80&auto=format&fit=crop" alt="Online" loading="lazy">
        <div class="galeri-ov"><span>Kelas Online</span></div>
      </div>
      <div class="galeri-item">
        <img src="https://images.unsplash.com/photo-1488521787991-ed7bbaae773c?w=600&q=80&auto=format&fit=crop" alt="Wisuda" loading="lazy">
        <div class="galeri-ov"><span>Wisuda Siswa</span></div>
      </div>
      <div class="galeri-item">
        <img src="https://images.unsplash.com/photo-1476357471311-43c0db9fb2b4?w=600&q=80&auto=format&fit=crop" alt="Prestasi" loading="lazy">
        <div class="galeri-ov"><span>Perayaan Prestasi</span></div>
      </div>
    </div>
    <div style="text-align:center;margin-top:40px">
      <a href="#" style="display:inline-flex;align-items:center;gap:9px;background:var(--ink);color:#fff;padding:14px 34px;border-radius:12px;font-weight:700;font-size:14px;transition:all .28s" onmouseover="this.style.background='var(--violet)';this.style.transform='translateY(-2px)'" onmouseout="this.style.background='var(--ink)';this.style.transform='translateY(0)'">
        Lihat Semua Galeri →
      </a>
    </div>
  </div>
    ...
    <div class="tutor-grid reveal">
      <div class="tutor-card">
        <div class="tutor-img-wrap">
          <img src="https://images.unsplash.com/photo-1607746882042-944635dfe10e?w=200&h=200&q=80&auto=format&fit=crop&crop=face" class="tutor-img" alt="Anisa" loading="lazy">
          <div class="tutor-badge">★</div>
        </div>
        <div class="tutor-name">Ms. Anisa Putri</div>
        <div class="tutor-subj">Matematika</div>
        <div class="tutor-rating">★ 4.9</div>
        <div class="tutor-exp">7 Tahun Pengalaman</div>
      </div>
      <div class="tutor-card">
        <div class="tutor-img-wrap">
          <img src="https://images.unsplash.com/photo-1506794778202-cad84cf45f1d?w=200&h=200&q=80&auto=format&fit=crop&crop=face" class="tutor-img" alt="Budi" loading="lazy">
          <div class="tutor-badge">★</div>
        </div>
        <div class="tutor-name">Mr. Budi Santoso</div>
        <div class="tutor-subj">Fisika</div>
        <div class="tutor-rating">★ 4.8</div>
        <div class="tutor-exp">9 Tahun Pengalaman</div>
      </div>
      <div class="tutor-card">
        <div class="tutor-img-wrap">
          <img src="https://images.unsplash.com/photo-1580489944761-15a19d654956?w=200&h=200&q=80&auto=format&fit=crop&crop=face" class="tutor-img" alt="Cindy" loading="lazy">
          <div class="tutor-badge">★</div>
        </div>
        <div class="tutor-name">Ms. Cindy Lestari</div>
        <div class="tutor-subj">Bahasa Inggris</div>
        <div class="tutor-rating">★ 4.9</div>
        <div class="tutor-exp">6 Tahun Pengalaman</div>
      </div>
      <div class="tutor-card">
        <div class="tutor-img-wrap">
          <img src="https://images.unsplash.com/photo-1472099645785-5658abf4ff4e?w=200&h=200&q=80&auto=format&fit=crop&crop=face" class="tutor-img" alt="Dimas" loading="lazy">
          <div class="tutor-badge">★</div>
        </div>
        <div class="tutor-name">Mr. Dimas Arif</div>
        <div class="tutor-subj">Akuntansi</div>
        <div class="tutor-rating">★ 4.8</div>
        <div class="tutor-exp">8 Tahun Pengalaman</div>
      </div>
      <div class="tutor-card">
        <div class="tutor-img-wrap">
          <img src="https://images.unsplash.com/photo-1573496359142-b8d87734a5a2?w=200&h=200&q=80&auto=format&fit=crop&crop=face" class="tutor-img" alt="Rina" loading="lazy">
          <div class="tutor-badge">★</div>
        </div>
        <div class="tutor-name">Ms. Rina Wulandari</div>
        <div class="tutor-subj">Kimia</div>
        <div class="tutor-rating">★ 4.9</div>
        <div class="tutor-exp">6 Tahun Pengalaman</div>
      </div>
    </div>
        ...
        <div class="contact-box">
          ...
          <div class="f-row">
            ...
            <div class="f-group">
              ...
              <input type="text" placeholder="08xxxxxxxxxx" id="inp-wa">
            </div>
          </div>
          <div class="f-row">
            <div class="f-group">
              <label>Email</label>
              <input type="email" placeholder="email@gmail.com" id="inp-email">
            </div>
            <div class="f-group">
              <label>Kota / Cabang</label>
              <select id="inp-kota">
                <option value="">Pilih kota</option>
                <option>Medan</option><option>Jakarta</option>
                <option>Bandung</option><option>Surabaya</option>
                <option>Pekanbaru</option><option>Palembang</option>
                <option>Jambi</option><option>Aceh</option>
    ...
    <div class="cabang-grid reveal">
      <div class="cabang-card">
        <div class="cabang-img-wrap">
          <img src="https://images.unsplash.com/photo-1596422846543-75c6fc197f07?w=600&q=80&auto=format&fit=crop" class="cabang-img" alt="Medan" loading="lazy">
        </div>
        <div class="cabang-info">
          <h3>Medan</h3>
          <p>Jasa Les Privat Medan</p>
      ...
      <div class="cabang-card">
        <div class="cabang-img-wrap">
          <img src="https://images.unsplash.com/photo-1555400038-63f5ba517a47?w=600&q=80&auto=format&fit=crop" class="cabang-img" alt="Palembang" loading="lazy">
        </div>
        <div class="cabang-info">
          <h3>Palembang</h3>
          <p>Jasa Les Privat Palembang</p>
          <button class="cabang-btn">Lihat Detail</button>
        </div>
      </div>
      <div class="cabang-card">
        <div class="cabang-img-wrap">
          <img src="https://images.unsplash.com/photo-1564596823821-79b335a76a82?w=600&q=80&auto=format&fit=crop" class="cabang-img" alt="Aceh" loading="lazy">
        </div>
        <div class="cabang-info">
          <h3>Aceh</h3>
          <p>Jasa Les Privat Aceh</p>
          <button class="cabang-btn">Lihat Detail</button>
        </div>
      </div>
      <div class="cabang-card">
        <div class="cabang-img-wrap">
          <img src="https://images.unsplash.com/photo-1529016977882-5017d69f43da?w=600&q=80&auto=format&fit=crop" class="cabang-img" alt="Jakarta" loading="lazy">
        </div>
        <div class="cabang-info">
          <h3>Jakarta</h3>
          <p>Jasa Les Privat Jakarta</p>
          <button class="cabang-btn">Lihat Detail</button>
        </div>
      </div>
      <div class="cabang-card">
        <div class="cabang-img-wrap">
          <img src="https://images.unsplash.com/photo-1568632234157-ce7aecd03d0d?w=600&q=80&auto=format&fit=crop" class="cabang-img" alt="Bandung" loading="lazy">
        </div>
        <div class="cabang-info">
          <h3>Bandung</h3>
          <p>Jasa Les Privat Bandung</p>
          <button class="cabang-btn">Lihat Detail</button>
        </div>
      </div>
      <div class="cabang-card">
        <div class="cabang-img-wrap">
          <img src="https://images.unsplash.com/photo-1556001934-b7f70a99c11c?w=600&q=80&auto=format&fit=crop" class="cabang-img" alt="Surabaya" loading="lazy">
        </div>
        <div class="cabang-info">
          <h3>Surabaya</h3>
          <p>Jasa Les Privat Surabaya</p>
          <button class="cabang-btn">Lihat Detail</button>
        </div>
      </div>
      <div class="cabang-card">
        <div class="cabang-img-wrap">
          <img src="https://images.unsplash.com/photo-1598257007024-8c61c7e2c72f?w=600&q=80&auto=format&fit=crop" class="cabang-img" alt="Makassar" loading="lazy">
        </div>
        <div class="cabang-info">
          <h3>Makassar</h3>
          <p>Jasa Les Privat Makassar</p>
          <button class="cabang-btn">Lihat Detail</button>
        </div>
      </div>
    </div>
...
if(heroBg){heroBg.onload=()=>heroBg.classList.add('loaded')}
...
// ── JENJANG TOGGLE
function toggleJenjang(card){
  const was=card.classList.contains('open')
  document.querySelectorAll('.jenjang-card').forEach(c=>c.classList.remove('open'))
...
function toggleProg(card){
  const was=card.classList.contains('open')
  document.querySelectorAll('.prog-card').forEach(c=>c.classList.remove('open'))
  if(!was)card.classList.add('open')
}
